...
function DH_getWorkspaceData()
{
	...
	var respstring = AJX_RequestWithReponseData("text", "WKSM", "101", "");
	if (!respstring) {
...
{
	 var respstring  = AJX_RequestWithReponseData("text", "WKSM", "104", NodeID);
     if (!respstring) {
...
{
	 var respstring = AJX_RequestWithReponseData("text", "WKSM", "103", ProjectID);
	 if (!respstring) {
	        Debug_Message("could not get data");
	        return null;	    
	 }
...
function DH_getnewPage(PageTitle)
{
    ...
    var respstring = AJX_RequestWithReponseData("text", "PRJM", "201", PageTitle);
    if (!respstring) {
...
{
	var respstring = AJX_RequestWithReponseData("text", "PRJM", "203", PageID);
	if (!respstring) {
	      Debug_Message("could not get data");
	      return null;
	}
...
{
	var respstring = AJX_RequestWithReponseData("text", "PRJM", "204", PageID);
	if (!respstring) {
	      Debug_Message("could not get data");
	      return null;
	}
...
{
	var respstring = AJX_RequestWithReponseData("text", "PRJM", "202", PageID);
    if (!respstring) {
...
function DH_getPageData(PageID)
{
	...
	var respstring = AJX_RequestWithReponseData("text", "PRJM", "206", PageID);
    if (!respstring) {
...
function DH_getPageHTMLData(PageID)
{
	 ...
	 var respstring = AJX_RequestWithReponseData("text", "PRJM", "205", PageID);
     if (respstring == "Error") {
	        Debug_Message("could not get data");
	        return null;
	}
...
function DH_AddNewObject(newObjectType, ObjectID, parentObjectID, ObjectName, dataType)
{
	...
	var respstring = ""; 
	respstring = AJX_RequestWithReponseData("text", "OBJM", "301", ObjParam);
...
{
	var ObjParam = PageID;
	var respstr = AJX_RequestWithReponseData("text", "OBJM", "300", ObjParam);
	return respstr; 
}
...
function DH_MoveNode(currentNode, direction) {
   ...
   else 
   {
    ...
    var respstring = "";
    respstring = AJX_RequestWithNoReponseData("text", "OBJM", reqcode, ObjParam);
...
function DH_deleteObjectNode(currentNode) {
   ...
   //currNodeID = currNodeID.substring(3); 
    ...
    var respstring = "";
    respstring = AJX_RequestWithReponseData("text", "OBJM", "302", ObjParam);
...
function DH_SaveEditList(finalEditList) {
  ...
  //  var finalEditList = UIH_getFinalEditList(gEditList); 
   ...
   // for (var n = 0; n < listlen - 1; n++) 
    ...
    finalEditList = DH_DeleteFromEditList(finalEditList, 0); 
}
...
function DH_getPageURL()
{
    ...
    var respstring = AJX_RequestWithReponseData("text", "PRJM", "207");
    if (respstring == "FAIL") {
        return null; 
    }    
    return respstring; 
...
{
	var arrvar; 
	var respstring = AJX_RequestWithReponseData("text", "OBJM", "310");
...
{
	var respstring = AJX_RequestWithReponseData("text", "OBJM", "309");
    if (!respstring) {
...
{
	var respstring = AJX_RequestWithReponseData("text", "OBJM", "311");
    if (!respstring) {
...
function DH_setTreeItemAttribute(nodeID, attrname, attrVal)
{
	var localnode =  document.getElementById(nodeID);
	if(localnode)
		localnode.setAttribute(attrname, attrVal);
	
	var reqbody = "&NODEID=" + nodeID + "&ATTRNAME=" + attrname + "&ATTRVALUE=" + attrVal ;
	var respstring = AJX_RequestWithReponseData("text", "OBJM", "311", reqbody);
}

function DH_getAssetListFromServer(assetType)
{
	var respstring = AJX_RequestWithReponseData("text", "PRJM", "210", assetType);
    if (!respstring) {
...
function DH_deleteAssetFile(fname)
{
	var respstring = AJX_RequestWithNoReponseData("text", "PRJM", "211", fname);    
}
...
{
	var respstring = AJX_RequestWithNoReponseData("text", "PRJM", "208",PageID);
	if (respstring != 'OK') {
	      Debug_Message("DH_updateSlideShowInfo Error" );
	      return null;
	}
	return respstring; 
}
...
function DH_updateHTMLObject(ObjectID, dataType, HTMLstr, bAsync)
{
	 ...
	 {
		 var respstring = AJX_RequestWithReponseData("text", "OBJM", "307", reqbody);
		 if (respstring != "OK") {
		      Debug_Message("DH_updateHTMLObject Error" + reqbody);
		      return null;
		  }	 
		 return respstring; 
	 }
...
function DH_addLayerReference(layerID, parentID)
{
	...
	//_rm in case of Ref layer the unqiueness should be searched in the tree items only 
        ...
        var respstr = AJX_RequestWithReponseData("text", "OBJM", "306", ObjParam);
        if (!respstr) 
        {
            Debug_Message("Failed to Add Layer data due to some error");
            return null;
        }
        gRefLayerIDAdded = newObjID; 
        return respstr; 
        //return newObjID; 
}
...
function DH_moveObject(objectID, destParentID, objectType, objPosition )
{
    ...
    var respstr = AJX_RequestWithNoReponseData("text", "OBJM", "313", ObjParam);
    if (respstr != "OK") {
...
{
	var ObjParam = "OBJECTID=" + objectID + "&DESTOBJECTID=" + destParentID + "&TYPE=" + objectType;
    var respstr = AJX_RequestWithNoReponseData("text", "OBJM", "314", ObjParam);
    if (respstr != "OK") {
...
{
	var finalEditList = UIH_getFinalEditList(gEditList, 0); 
	//var listlen = finalEditList.length;
	return finalEditList; 
}

function DH_UpdateTreeNodeText(nodeID, nodeText, bAsync)
{
	var ObjParam = "NODEID=" + nodeID + "&NODETEXT=" + nodeText;
	if(bAsync == true)
	{
		var respstr = AJX_RequestWithNoReponseData("text", "OBJM", "315", ObjParam);
		return ; 
	}
	else
	{
		respstr = AJX_RequestWithReponseData("text", "OBJM", "315", ObjParam);
		if (respstr != "OK") {
	        Debug_Message("Failed to Update Node Title");
	        return 'FAIL'; 
	}
...
function DH_UpdateAutoSlideShowInfo(pageID)
{
	var ObjParam = pageID;
	var respstr = AJX_RequestWithNoReponseData("text", "OBJM", "316", ObjParam);
	if(respstr != 'OK')
	{
		Debug_Message("Error in Updating Auto Show Info"); 
	}
}
function DH_UpdateSlideEffectInfo(pageID)
{
	var ObjParam = pageID;
	var respstr = AJX_RequestWithNoReponseData("text", "OBJM", "317", ObjParam);
	if(respstr != 'OK')
	{
		Debug_Message("Error in Updating Slide Effect Info"); 
	}
}
